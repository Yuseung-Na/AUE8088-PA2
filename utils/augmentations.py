...
def random_perspective_rgbt(
    ims, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0)
):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(0.1, 0.1), scale=(0.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
...
def mixup_rgbt(ims, labels, im2, labels2):
    """
    Applies MixUp augmentation by blending images and labels.
    ...
    r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0
    for i in range(len(ims)):
        ims[i] = (ims[i] * r + im2[i] * (1 - r)).astype(np.uint8)
    labels = np.concatenate((labels, labels2), 0)
    return ims, labels
